...
const Notes = () => {
  const [adminActiveUserId, adminSetActiveUserId] = useState([]);
  ...
  const receivedUserId = location.state.passedUserId;
  ...
  async function fetchNotes() {
    ...
    adminSetActiveUserId(receivedUserId);
  }
  ...
  const showMapHandler = (e) => {
    ...
    fetch(
      ...
      .then((data) => currentPosition.push(data));
    console.log(currentPosition);

    if (currentPosition.length > 0) {
      const passedCoords = currentPosition;
      setTimeout(() => {
        navigate2("/position", { state: { passedCoords: currentPosition[0] } });
      }, 1000);
    } else {
      setTimeout(() => {
  ...
  return (
    <div className="notes-body">
      {addNoteClicked && (
        <AdminAddNote
          sentActiveUserId={adminActiveUserId}
